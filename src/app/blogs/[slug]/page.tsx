   ...
   const content = blogContents![post.id] || [];
    ...
    return (
      <div className=''>
        ...
        <h1 className="text-4xl text-dark_grey font-sen  md:text-5xl font-bold mb-4 text-center">{post.title}</h1>
        <div className='flex justify-center flex-col px-10 md:20'>
        <p className="text-gray-600 mb-4 text-center ">
             <span className="font-semibold text-purple"> By {post.author}</span> on {post.date}
        </p>
        ...
        {content.map((item, index) => {
            if (item.type === "paragraph") {
              return <p key={index} className='mt-4 font-inter text-xl text-justify '>{item.content}</p>;
            } else if (item.type === "image") {
              return <img key={index} src={item.src} alt={item.alt} className="my-8 w-full h-auto " />;
            } else if (item.type === "heading") {
              const HeadingTag = `h${item.level}` as keyof JSX.IntrinsicElements;
              return React.createElement(HeadingTag, { key: index, className: `mt-16 md:text-${item.level === 1 ? '5xl' : '4xl'} text-${item.level === 1 ? '3xl' : '2xl'} font-sen font-bold` }, item.content);
